  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-slate-100 flex flex-col font-sans selection:bg-emerald-500 selection:text-black">
      ...
      <header className="sticky top-0 z-50 bg-[#09090B]/90 backdrop-blur-xl border-b border-slate-800/80 px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-2xl">
        ...
        <CourseSwitcher currentLanguage="c" currentChapter={currentOrderNum} />
      </header>

      {/* Main Body */}
      <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-8 space-y-8">
        <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-2">
          <span className="text-xs font-mono font-bold text-emerald-400">C Assessment &bull; Chapter {currentOrderNum}</span>
        ...
        {loading ? (
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900 border border-slate-800 space-y-6 text-center shadow-2xl">
            ...
            <div className="flex flex-wrap justify-center gap-4 pt-4 border-t border-slate-800">
              ...
              {result.passed && currentOrderNum < 10 && (
                <button
                  onClick={() => router.push(`/courses/c/chapter/${currentOrderNum + 1}`)}
                  className="px-6 py-3 rounded-xl bg-emerald-600 text-white text-xs font-black hover:bg-emerald-500 transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/20"
                >
            ...
            {questions.map((q, idx) => (
              <div key={q.id} className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-4">
                <h3 className="text-base font-bold text-white flex items-start gap-3">
                  <span className="w-6 h-6 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center text-xs font-mono shrink-0">
                    {idx + 1}
                  </span>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-800/80">
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-semibold px-1">
                <span className="text-slate-400">
                  Assessment Progress: <strong className="text-emerald-400 font-mono">{Object.keys(answers).length} / {questions.length}</strong> Answered
                </span>
              ...
              <button
                ...
                className={`w-full py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-2 shadow-xl ${
                  Object.keys(answers).length < questions.length || submitting
                    ...
                    : "bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 hover:brightness-110 text-white shadow-emerald-500/25 active:scale-[0.99] cursor-pointer"
                }`}
              >
                {submitting
                  ? "Evaluating Answers..."
