...
1. Why must the `main` method be declared as `static` in Java?
   - A) To prevent other classes from modifying the main method.
   - B) To allow the JVM to invoke the entry point without creating an object instance of the class.
   - C) To speed up string concatenation inside the method.
   - D) To force the method to return an integer error code.
   *Correct Answer: B*

2. What is the output artifact generated by running `javac Program.java` on a file containing 4 classes?
   - A) 1 single `.exe` file
   - B) 1 single `.class` file containing all 4 classes
   - C) 4 separate `.class` files
   - D) 4 separate `.jar` archives
   *Correct Answer: C*

#### Practice Exercise
Create a program `CommandLineCalculator.java` that accepts two numbers as command-line arguments (`args[0]` and `args[1]`), converts them to integers using `Integer.parseInt()`, and prints their sum.
